    ...
    return (
        <div>
            <div className={style.tabContainer}>
                <button className={activeTab === "search" ? style.activeTab : style.tab} onClick={() => setActiveTab("search")}>친구 검색</button>
                <button className={activeTab === "following" ? style.activeTab : style.tab} onClick={() => setActiveTab("following")}>팔로잉</button>
                <button className={activeTab === "followers" ? style.activeTab : style.tab} onClick={() => setActiveTab("followers")}>팔로워</button>
            </div>
            <div className={style.userListContainer}>
                ...
                {activeTab === "following" && (
                    <div className={style.userListContainer}>
                        {followingData.length > 0 ? followingData.map((email, index) => (
                            <div key={index} className={style.userItem}>{email}</div>
                        )) : '팔로우한 사용자가 없습니다.'}
                    </div>
                )}
                {activeTab === "followers" && (
                    <div className={style.userListContainer}>
                        {followerData.length > 0 ? followerData.map((email, index) => (
                            <div key={index} className={style.userItem}>{email}</div>
                        )) : '나를 팔로우한 사용자가 없습니다.'}
                    </div>
                )}
